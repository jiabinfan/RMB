...
def obtain_bon_gold_score():
    ...
    # root_path = script_args.data_path
    # pattern = str(Path(root_path) / "batch_*")
    # for data_path in glob.glob(pattern):
    ...
    if accelerator.is_main_process:
        ...
        df = df.groupby("order_ids", as_index=False)["gold_rewards"].mean()
        df = df.drop_duplicates(subset=["order_ids"])
        df.to_csv(os.path.join(script_args.data_path, 'gold_score.csv'), index=False)
